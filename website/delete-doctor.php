...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if ($id > 0) {
        // Prepare and bind
        $stmt = $conn->prepare("DELETE FROM doctor WHERE dr_id = ?"); // Change to doctor table and dr_id
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Error deleting doctor: ' . $stmt->error]);
        }
